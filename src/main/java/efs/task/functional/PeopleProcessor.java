...
class PeopleProcessor {
    ...
    List<String> namesOfPeopleWhoseAgeIsGreaterThan(int age, Collection<Person> people) {
        return people.stream().filter(p -> p.getAge() > age).map(p -> p.getName()).collect(Collectors.toList());
    }
    ...
    List<String> namesOfPeopleSortedByAgeLivingIn(String cityOfLiving, Collection<Person> people) {
        return people.stream().filter(p -> p.getCityOfLiving().equals(cityOfLiving)).sorted(Comparator.comparing(Person::getAge)).map(p -> p.getName()).collect(Collectors.toList());
    }
    ...
    Map<String, Double> averageAgeByCityOfLiving(Collection<Person> people) {
        return people.stream().map(p -> p.getCityOfLiving()).distinct().collect(Collectors.toMap(c -> c, c -> people.stream().filter(p -> p.getCityOfLiving().equals(c)).mapToInt(Person::getAge).average().getAsDouble()));
    }
}
